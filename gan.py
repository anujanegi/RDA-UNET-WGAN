...
def build_discriminator():
    ...
    conv1 = Conv2D(n_filters, kernel_size=(k, k), strides=(s, s), padding="same")(
        inputs
    )
    ...
    conv2 = Conv2D(2 * n_filters, kernel_size=(k, k), strides=(s, s), padding="same")(
        pool1
    )
...
def train():
    ...
    for n_round in range(n_rounds):
        ...
        for i in range(steps_per_epoch_d):
            ...
            img_discr_batch, lab_discr_batch = imgs2discr(
                image_batch, labels_batch, pred
            )
            (
                loss,
                acc,
                ...
                recall,
                mean_iou,
            ) = model_discriminator.train_on_batch(img_discr_batch, lab_discr_batch)
        ...
        for i in range(steps_per_epoch_g):
            ...
            (
                ...
                recall,
                mean_iou,
            ) = model_gan.train_on_batch(img_gan_batch, lab_gan_batch)
        ...
        for i in range(steps_val_d):
            ...
            img_discr_val, lab_discr_val = imgs2discr(
                image_val_batch, labels_val_batch, pred
            )
            (
                loss,
                acc,
                ...
                recall,
                mean_iou,
            ) = model_discriminator.test_on_batch(img_discr_val, lab_discr_val)
        ...
        for i in range(steps_val_g):
            ...
            (
                ...
                recall,
                mean_iou,
            ) = model_gan.test_on_batch(img_gan_val, lab_gan_val)
        ...
        model_gan.save_weights(
            os.path.join(
                path_to_trained_generator_weights + "/gan/",
                "gan_weights_{}.h5".format(n_round),
            )
        )
        model_generator.save_weights(
            os.path.join(
                path_to_trained_generator_weights + "/unet/",
                "unet_weights_{}.h5".format(n_round),
            )
        )
