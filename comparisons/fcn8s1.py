...
from keras.layers import Input, concatenate, Convolution2D, MaxPooling2D, UpSampling2D,Conv2D, Dropout, BatchNormalization
...
def get_unet():
    ...
    up7 = concatenate([UpSampling2D(size=(2, 2))(conv5), pool3], axis=3)
    up8 = concatenate([UpSampling2D(size=(2, 2))(up7), pool2], axis=3)
    ...
    conv10 = Conv2D(1, (1, 1), activation='sigmoid')(up8)
    #BatchNormalization(axis=3)

    model = Model(inputs=[inputs], outputs=[conv10])

    model.compile(optimizer=Adam(1e-5), loss=dice_coef_loss, metrics=['accuracy',dice_coef,sensitivity,specificity,f1score,precision,recall, mean_iou])
...
def train():
    ...
    print('-' * 30)
    model = get_unet()
    model_checkpoint = ModelCheckpoint('/home/NanLi/ceshi/unsmceshi/u_net/raw/file/fcn8s.hdf5', monitor='val_loss',
                                       save_best_only=True)
