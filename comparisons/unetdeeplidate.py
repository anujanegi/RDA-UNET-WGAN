...
def build_discriminator():
    ...
    # model = []
    path_1 = Conv2D(filters=32, kernel_size=(3, 3), padding='same', strides=(1, 1))(inputs)
    path_1 = LeakyReLU(alpha=0.2)(path_1)
    path_1 = Conv2D(filters=32, kernel_size=(3, 3), padding='same', strides=(1, 1))(path_1)
    ...
    model_.compile(optimizer=Adam(0.0002, 0.5), loss='binary_crossentropy', metrics=['accuracy',dice_coef, sensitivity,specificity,f1score,precision,recall,mean_iou])
    return model_
...
def train():
    ...
    model_checkpoint = ModelCheckpoint('./raw/file/RDAunet.hdf5', monitor='val_loss',
                                       save_best_only=True)
    ...
    combined.compile(optimizer=Adam(0.0002, 0.5), loss='binary_crossentropy', metrics=['accuracy',dice_coef, sensitivity,specificity,f1score,precision,recall,mean_iou])
